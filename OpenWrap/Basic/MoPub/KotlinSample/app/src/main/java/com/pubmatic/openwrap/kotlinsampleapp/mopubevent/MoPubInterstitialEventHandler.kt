 ...
 * unit. It implements the PubMatic's Wrapper interface. OpenWrap SDK notifies (using wrapper interface)
...
open class MoPubInterstitialEventHandler(
    ...
    override fun requestAd(bid: POBBid?) {
        ...
        if (!isConnected(context)) {
            ...
            PMLog.error(MoPubInterstitialEventHandler.TAG, error.toString())
        ...
        if (moPubInterstitial?.interstitialAdListener != this) {
            moPubInterstitial?.setInterstitialAdListener(this)
            PMLog.warn(TAG, "Resetting MoPub interstitial interstitial to MoPubInterstitialEventHandler" +
    ...
    override fun onInterstitialLoaded(interstitial: MoPubInterstitial) {
        ...
        val bid : POBBid? = interstitial.localExtras[POBInterstitialCustomEvent.BID_KEY] as? POBBid
        if (bid != null) {
            if (bid.hasWon()){
    ...
    override fun onInterstitialFailed(interstitial: MoPubInterstitial, errorCode: MoPubErrorCode) {
        if (null != eventListener) {
            ...
            PMLog.error(TAG, "Can not call failure callback, POBInterstitialEventListener reference null. MoPub error:$errorCode")
        }
    }
