...
class AdLoader(val appContext:Context, val adSize: AdSize, val slotId: String,
    ...
    private fun createBanner() : POBBannerView {
        ...
        val eventHandler = DFPBannerEventHandler(appContext, gamAdUnitId, adSize)

        // Set config block to event handler in order to provide TAM's ads bid targeting to DFP
        eventHandler.setConfigListener(object : DFPBannerEventHandler.DFPConfigListener{
            override fun configure(adView: AdManagerAdView, builder: AdManagerAdRequest.Builder, bid: POBBid?) {
